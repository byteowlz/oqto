...
import { X, Mic, MicOff, Volume2, VolumeX, Settings2, ChevronDown } from 'lucide-react';
    ...
    >
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="sm" className="gap-1">
                {visualizers.find(v => v.id === settings.visualizer)?.name || 'Visualizer'}
      ...
      {showSettings && (
        <div className="p-4 border-b bg-muted/30 space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-xs font-medium text-muted-foreground">
              Interrupt after {settings.interruptWordCount} word{settings.interruptWordCount !== 1 ? 's' : ''} {settings.interruptWordCount === 0 ? '(disabled)' : ''}
            </label>
            <Slider
      ...
      <div className="p-4 border-t space-y-3">
        ...
        <p className="text-center text-xs text-muted-foreground">
          {voiceState === 'listening' && 'Speak now... (silence will auto-send)'}
