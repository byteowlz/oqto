...
import { AudioLines, MessageSquare, Keyboard, MicOff, Loader2 } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" side="top" className="w-56">
        <DropdownMenuItem onClick={onConversation} className="flex flex-col items-start gap-0.5 py-2">
          <div className="flex w-full items-center">
            <MessageSquare className="mr-2 size-4" />
            <span className="font-medium">{t.conversation}</span>
            <DropdownMenuShortcut>{shortcuts.conversation}</DropdownMenuShortcut>
          </div>
          <span className="text-xs text-muted-foreground pl-6">{t.conversationDesc}</span>
        </DropdownMenuItem>
        <DropdownMenuItem onClick={onDictation} className="flex flex-col items-start gap-0.5 py-2">
          <div className="flex w-full items-center">
            <Keyboard className="mr-2 size-4" />
            <span className="font-medium">{t.dictation}</span>
            <DropdownMenuShortcut>{shortcuts.dictation}</DropdownMenuShortcut>
          </div>
          <span className="text-xs text-muted-foreground pl-6">{t.dictationDesc}</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  )
