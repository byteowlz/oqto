...
import { MicOff, Volume2, VolumeX, Settings2, ChevronDown, ChevronUp, X } from 'lucide-react';
...
import type { VoiceState, VisualizerType, VoiceSettings } from '@/lib/voice/types';
      ...
      <div className="flex-shrink-0 border-t p-3 space-y-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="gap-1 h-8 px-2 text-xs">
                  {visualizers.find(v => v.id === settings.visualizer)?.name || 'Viz'}
              ...
              <DropdownMenuContent align="start">
                ...
                {visualizers.map(v => (
                  <DropdownMenuItem
                    ...
                    className={cn(
                      settings.visualizer === v.id && 'bg-accent'
                    )}
                  >
                    {v.name}
                  </DropdownMenuItem>
            ...
            <Button
              ...
              className={cn('h-8 w-8 p-0', settings.muted && 'text-muted-foreground')}
            >
            ...
            >
              {showSettings ? <ChevronUp className="size-4" /> : <Settings2 className="size-4" />}
            </Button>
          </div>
        ...
        {showSettings && (
          <div className="space-y-3 pt-2 border-t">
            ...
            <div className="space-y-1">
              <label className="text-xs font-medium text-muted-foreground">
                Interrupt: {settings.interruptWordCount === 0 ? 'off' : `${settings.interruptWordCount} word${settings.interruptWordCount !== 1 ? 's' : ''}`}
              </label>
              <Slider
