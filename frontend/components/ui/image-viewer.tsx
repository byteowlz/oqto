...
import { useCallback, useState, useRef, type WheelEvent } from "react"
...
export function ImageViewer({ src, alt, filename, className }: ImageViewerProps) {
    ...
    <div className={cn("flex flex-col h-full", className)}>
      ...
      <div className="flex items-center justify-between px-3 py-2 bg-muted border-b border-border shrink-0">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button variant="ghost" size="sm" onClick={handleReset} title={isFitToScreen ? "Actual size" : "Fit to screen"}>
            {isFitToScreen ? <Maximize2 className="w-4 h-4" /> : <Minimize2 className="w-4 h-4" />}
          </Button>
      ...
      <div
        ...
        style={{ cursor: isDragging ? "grabbing" : scale > 1 || !isFitToScreen ? "grab" : "default" }}
      >
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src={src}
          alt={alt || filename || "Image preview"}
          className={cn("transition-transform", isFitToScreen && "max-w-full max-h-full object-contain")}
          style={{
