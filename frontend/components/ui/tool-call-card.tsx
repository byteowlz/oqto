...
function isTodoList(input: Record<string, unknown>): input is { todos: TodoItem[] } {
  return Array.isArray(input.todos) && input.todos.length > 0 && 
    input.todos.every((t: unknown) => 
      typeof t === "object" && t !== null && 
      "content" in t && "status" in t
    )
}

function isFileRead(input: Record<string, unknown>): input is { filePath: string; offset?: number; limit?: number } {
  return typeof input.filePath === "string" && !("content" in input) && !("oldString" in input)
}

function isFileWrite(input: Record<string, unknown>): input is { filePath: string; content: string } {
  return typeof input.filePath === "string" && typeof input.content === "string" && !("oldString" in input)
}

function isFileEdit(input: Record<string, unknown>): input is { filePath: string; oldString: string; newString: string; replaceAll?: boolean } {
  return typeof input.filePath === "string" && typeof input.oldString === "string" && typeof input.newString === "string"
}

function isGlobSearch(input: Record<string, unknown>): input is { pattern: string; path?: string } {
  return typeof input.pattern === "string" && !("include" in input) && !("command" in input)
}

function isGrepSearch(input: Record<string, unknown>): input is { pattern: string; path?: string; include?: string } {
  return typeof input.pattern === "string" && !("command" in input)
}

function isBashCommand(input: Record<string, unknown>): input is { command: string; description?: string; workdir?: string; timeout?: number } {
  return typeof input.command === "string"
}

function isTaskCall(input: Record<string, unknown>): input is { prompt: string; description?: string; subagent_type?: string; session_id?: string } {
  return typeof input.prompt === "string" && (typeof input.subagent_type === "string" || typeof input.description === "string")
}

function isWebFetch(input: Record<string, unknown>): input is { url: string; format?: string; timeout?: number } {
  return typeof input.url === "string"
}

function isListDir(input: Record<string, unknown>): input is { path?: string; ignore?: string[] } {
  // List tool typically only has path and optionally ignore
  return (typeof input.path === "string" || Object.keys(input).length === 0) && 
    !("filePath" in input) && !("pattern" in input) && !("command" in input) && 
    !("prompt" in input) && !("url" in input) && !("todos" in input) &&
    !("content" in input) && !("oldString" in input)
}
...
function FileReadRenderer({ input }: { input: { filePath: string; offset?: number; limit?: number } }) {
  return (
...
function FileWriteRenderer({ input }: { input: { filePath: string; content: string } }) {
...
function FileEditRenderer({ input }: { input: { filePath: string; oldString: string; newString: string; replaceAll?: boolean } }) {
  const ext = getFileExtension(input.filePath)
...
function BashCommandRenderer({ input }: { input: { command: string; description?: string; workdir?: string; timeout?: number } }) {
  return (
    <div className="space-y-2">
      {input.description && (
...
function WebFetchRenderer({ input }: { input: { url: string; format?: string; timeout?: number } }) {
  return (
    <div className="space-y-2">
...
function ListDirRenderer({ input }: { input: { path?: string; ignore?: string[] } }) {
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 text-sm">
...
function FileOutputRenderer({ output, filePath }: { output: string; filePath?: string }) {
