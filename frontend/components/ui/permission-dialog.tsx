...
import { Shield, Terminal, FileEdit, Globe, AlertTriangle } from "lucide-react"
...
interface PermissionDialogProps {
  ...
  onRespond: (permissionId: string, response: PermissionResponse) => Promise<void>
  onDismiss: () => void
}

// Map tool names to icons and descriptions
function getToolInfo(tool: string): { icon: React.ReactNode; category: string; riskLevel: "low" | "medium" | "high" } {
  const toolLower = tool.toLowerCase()
  
  if (toolLower.includes("bash") || toolLower.includes("shell") || toolLower.includes("exec")) {
    return { icon: <Terminal className="w-5 h-5" />, category: "Shell Command", riskLevel: "high" }
  }
  if (toolLower.includes("edit") || toolLower.includes("write") || toolLower.includes("file")) {
    return { icon: <FileEdit className="w-5 h-5" />, category: "File Edit", riskLevel: "medium" }
  }
  if (toolLower.includes("web") || toolLower.includes("fetch") || toolLower.includes("http")) {
    return { icon: <Globe className="w-5 h-5" />, category: "Web Request", riskLevel: "low" }
  }
  
  return { icon: <Shield className="w-5 h-5" />, category: "Tool", riskLevel: "medium" }
}
...
export function PermissionDialog({ permission, onRespond, onDismiss }: PermissionDialogProps) {
  const [isResponding, setIsResponding] = useState(false)
  const [selectedResponse, setSelectedResponse] = useState<PermissionResponse | null>(null)

  const handleRespond = useCallback(async (response: PermissionResponse) => {
      ...
      <DialogContent className="sm:max-w-md" showCloseButton={false}>
        <DialogHeader>
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <DialogTitle className="text-base">Permission Required</DialogTitle>
              <p className="text-xs text-muted-foreground mt-0.5">{toolInfo.category}</p>
            </div>
          </div>
          ...
          {riskLevel === "high" && (
            <div className="flex items-start gap-2 p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
              ...
              <p className="text-xs text-red-500">
                This action can execute arbitrary commands on the system. Review carefully before approving.
              </p>
            </div>
          )}
        </div>
