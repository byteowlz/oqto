...
export function PersonaBuilderChat({
  ...
  useEffect(() => {
    ...
    fetchSessions(opencodeBaseUrl)
      .then((allSessions) => {
        ...
        const builderOnes = allSessions.filter(s => s.title?.startsWith(SESSION_TITLE_PREFIX))
        if (builderOnes.length > 0) {
          const sorted = [...builderOnes].sort((a, b) => b.time.updated - a.time.updated)
  ...
  const handleNewSession = useCallback(async () => {
    ...
    try {
      ...
      const session = await createSession(opencodeBaseUrl, `${SESSION_TITLE_PREFIX} - ${timestamp}`)
  ...
  const handleSend = useCallback(async () => {
    ...
    const optimisticMessage: OpenCodeMessageWithParts = {
      ...
      parts: [{ id: `temp-part-${Date.now()}`, sessionID: sessionId, messageID: `temp-${Date.now()}`, type: "text", text: messageInput.trim() }],
    }
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {isLoading ? (
          ...
          <div className="text-center text-muted-foreground text-xs py-8">
            ...
            <p className="text-[10px]">Example: "Create a code reviewer that focuses on Python best practices"</p>
          </div>
        ) : (
          messages.map((msg) => {
            ...
            const displayText = isUser ? text.replace(/^@persona-builder\s*/i, "") : text
