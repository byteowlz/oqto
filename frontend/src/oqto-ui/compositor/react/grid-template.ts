...
export function gridTemplateFromSizes(sizes: readonly number[]): string {
	return sizes.length === 0 ? "none" : sizes.map((size) => `${size}px`).join(" ");
}

/** grid-row / grid-column line syntax for a placement, offset by `rowOffset`. */
export function gridLines(placement: GridPlacement, rowOffset: number): { readonly row: string; readonly column: string } {
	return {
