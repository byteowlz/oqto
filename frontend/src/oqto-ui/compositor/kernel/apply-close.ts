...
export function applyClose(
	...
	const working = replaceContainer(
		state,
		removeFromStack(container, contentId),
	);
	const settled = settleEmptiedContainer(working, container.id);
	const events: LayoutEvent[] = [
		{ type: "closed", contentId, containerId: container.id },
	];
	if (settled.removed) {
...
export function applyUndo(
	...
	if (snapshot.schemaVersion !== state.schemaVersion) {
		return {
			...
			rejection: {
				...
				detail: "undo target snapshot has a different schema version",
			},
		};
	}
	...
	return {
		...
		state: {
			...
			idSeed: Math.max(state.idSeed, snapshot.idSeed),
		},
