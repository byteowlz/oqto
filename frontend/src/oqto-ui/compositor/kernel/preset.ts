...
import { arrangementIdFrom, containerIdFrom, type ContentId, trackIdFrom } from "./ids";
import type { Container, ContentRef, GridPlacement, GridTrack, LayoutSnapshot, TrackSize } from "./model";
...
export function createClassicPresetLayout(content: ClassicPresetContent): LayoutSnapshot {
	const regions: readonly PresetRegion[] = [
		{ role: "navigation", emptyBehavior: "collapse", size: { unit: "fixed", value: 320, min: 240 }, stack: content.navigation },
		{ role: "primary", emptyBehavior: "retain", size: { unit: "fraction", value: 2, min: 360 }, stack: content.primary },
		{ role: "auxiliary", emptyBehavior: "retain", size: { unit: "fraction", value: 1, min: 280 }, stack: content.auxiliary },
	];
	const row: GridTrack = { id: trackIdFrom(1), size: { unit: "fraction", value: 1 } };
	...
	regions.forEach((region, index) => {
		...
		placements.push({ containerId, row: 0, column: index, rowSpan: 1, colSpan: 1 });
		containers.push({
			id: containerId,
			role: region.role,
	...
	const focusedContentId: ContentId | null =
		containers.find((container) => container.role === "primary")?.activeContentId ??
		containers.find((container) => container.activeContentId !== null)?.activeContentId ??
		null;
	return {
		schemaVersion: LAYOUT_SCHEMA_VERSION,
		revision: 0,
