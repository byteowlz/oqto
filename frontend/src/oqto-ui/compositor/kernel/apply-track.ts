...
export function applyResize(
	...
	const placement = arrangement
		? placementOf(arrangement.grid, containerId)
		: null;
	...
	if (!validTrackSize(size)) {
		return {
			...
			rejection: {
				...
				detail: "resize requires a finite positive track size",
			},
		};
	}
	...
	const track =
		axis === "inline"
			? arrangement.grid.columns[index]
			: arrangement.grid.rows[index];
	if (track.flush && size.unit !== "fixed") {
		return {
			ok: false,
			rejection: {
				reason: "invalid-command",
				detail: "a flush lane keeps a fixed size",
			},
		};
	}
	const grid = replaceTrack(
		arrangement.grid,
		axis === "inline" ? "column" : "row",
		index,
		{
			...track,
			size,
...
export function applyFlush(
	...
	if (index < 0) {
		return {
			ok: false,
			rejection: { reason: "invalid-command", detail: `unknown row ${rowId}` },
		};
	}
	if (flush && index !== 0 && index !== arrangement.grid.rows.length - 1) {
		return {
			...
			rejection: {
				reason: "invalid-command",
				detail: "only the first or last lane may be flush",
			},
		};
	}
	const spanning = arrangement.grid.placements.some(
		(placement) =>
			placement.row <= index &&
			index < placement.row + placement.rowSpan &&
			placement.rowSpan > 1,
	);
	if (flush && spanning) {
		return {
			...
			rejection: {
				reason: "invalid-command",
				detail: "a flush lane cannot be spanned by other placements",
			},
		};
	}
	const row = arrangement.grid.rows[index];
	const size =
		flush && row.size.unit !== "fixed" ? DEFAULT_FLUSH_SIZE : row.size;
	const { flush: _previous, ...rest } = row;
	const grid = replaceTrack(
		arrangement.grid,
		"row",
		index,
		flush ? { ...rest, size, flush: true } : { ...rest, size },
	);
	return {
		ok: true,
		state: replaceArrangement(state, { ...arrangement, grid }),
...
export function applyScroll(
	...
	if (
		...
		return {
			...
			rejection: {
				reason: "invalid-command",
				detail: `unknown column ${anchorColumnId}`,
			},
		};
	}
	...
	return {
		...
		state: replaceArrangement(state, {
			...arrangement,
			scrollAnchorColumnId: anchorColumnId,
		}),
		events: [
			{ type: "scrolled", arrangementId: arrangement.id, anchorColumnId },
		],
	};
}
