...
function checkTracks(
	...
	for (const [index, track] of tracks.entries()) {
		if (ids.has(track.id))
			push("track-identity", `duplicate track id ${track.id}`);
		ids.add(track.id);
		if (!validSize(track.size))
			push("track-size", `${label} ${track.id} has an invalid size`);
		if (track.flush) {
			if (index !== 0 && index !== tracks.length - 1) {
				push(
					"flush-edge",
					`flush lane ${track.id} is not the first or last row`,
				);
			}
...
function checkArrangement(
	...
	for (const placement of placements) {
		if (!containerIds.has(placement.containerId)) {
			push(
				"container-placement",
				`grid references unknown container ${placement.containerId}`,
			);
		}
		if (seenContainers.has(placement.containerId)) {
			push(
				"container-placement",
				`container ${placement.containerId} is placed more than once`,
			);
		}
		...
		if (
			...
			push(
				"span-bounds",
				`placement of ${placement.containerId} is out of bounds`,
			);
			continue;
		}
		for (let r = row; r < row + rowSpan; r += 1) {
			if (rows[r].flush && rowSpan !== 1) {
				push(
					"flush-confined",
					`placement of ${placement.containerId} spans into flush lane ${rows[r].id}`,
				);
			}
			coveredRows.add(r);
			for (let c = column; c < column + colSpan; c += 1) {
				...
				if (occupied.has(key)) {
					push(
						"placement-overlap",
						`placements overlap at row ${r}, column ${c}`,
					);
				}
				occupied.add(key);
			}
	...
	for (const [index, row] of rows.entries()) {
		if (!coveredRows.has(index))
			push("empty-track", `row ${row.id} has no placement`);
	}
	for (const [index, column] of columns.entries()) {
		if (!coveredColumns.has(index))
			push("empty-track", `column ${column.id} has no placement`);
	}
	if (
		...
		push(
			"scroll-anchor",
			`arrangement ${arrangement.id} anchors an unknown column`,
		);
	}
}
...
export function checkLayoutInvariants(
	...
	if (
		...
		(state.activeWorkspace.kind !== "workspace" ||
			state.activeWorkspace.id.length === 0)
	) {
	...
	for (const container of state.containers) {
		if (containerIds.has(container.id))
			push("container-identity", `duplicate container id ${container.id}`);
		containerIds.add(container.id);
		for (const content of container.stack) {
			if (seenContent.has(content.id)) {
				push(
					"content-single-container",
					`content ${content.id} is placed more than once`,
				);
			}
		...
		if (container.stack.length === 0) {
			if (container.activeContentId !== null) {
				push(
					"active-membership",
					`empty container ${container.id} has active content`,
				);
			}
		} else if (
			...
			push(
				"active-membership",
				`container ${container.id} active content is not in its stack`,
			);
		}
	}

	if (state.arrangements.length === 0)
		push("arrangements", "a layout needs at least one arrangement");
	...
	for (const arrangement of state.arrangements) {
		if (arrangementIds.has(arrangement.id))
			push(
				...
				`duplicate arrangement id ${arrangement.id}`,
			);
		...
		if (
			...
			!arrangement.grid.placements.some(
				(placement) =>
					contentContainer.get(arrangement.lastFocusedContentId as string) ===
					placement.containerId,
			)
		) {
			push(
				"focus-memory",
				`arrangement ${arrangement.id} remembers unplaced focus`,
			);
		}
	}
	for (const id of containerIds) {
		if (!seenContainers.has(id))
			push(
				"container-placement",
				`container ${id} is not placed in any arrangement`,
			);
	}

	const active = state.arrangements.find(
		(arrangement) => arrangement.id === state.activeArrangementId,
	);
	if (!active) {
		push(
			"active-arrangement",
			`active arrangement ${state.activeArrangementId} does not exist`,
		);
		return violations;
	}
	const activeContainers = new Set<string>(
		active.grid.placements.map((placement) => placement.containerId),
	);
	...
	if (activeContent.length === 0) {
		if (state.focusedContentId !== null) {
			push(
				"focus-empty",
				"an empty active arrangement must not have a focused target",
			);
		}
	} else if (
		state.focusedContentId === null ||
		!activeContent.includes(state.focusedContentId)
	) {
		push(
			"focus-reachable",
			`focus ${String(state.focusedContentId)} is not placed in the active arrangement`,
		);
	}
	return violations;
}
