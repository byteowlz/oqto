...
export function flattenForkDescendants(
	childrenByParent: Map<string, ChatSession[]>,
	parentId: string,
	depth = 1,
	seen = new Set<string>(),
): Array<{ session: ChatSession; depth: number }> {
	const result: Array<{ session: ChatSession; depth: number }> = [];
	seen.add(parentId);
	for (const child of childrenByParent.get(parentId) ?? []) {
		...
		result.push(
			...flattenForkDescendants(childrenByParent, child.id, depth + 1, seen),
		);
	}
	return result;
}
...
export interface SidebarSessionsProps {
	...
	/** If false, hide the sessions list (keeps header/actions visible). */
		...
		for (const project of sessionsByProject) {
			...
			for (const session of project.sessions) {
				...
				if (hasChildren && isExpanded) {
					for (const { session: descendant } of flattenForkDescendants(
						sessionHierarchy.childSessionsByParent,
						session.id,
					)) {
						if (hiddenSessionIds.has(descendant.id)) continue;
						...
						{onToggleSessionsExpanded && (
							...
							>
								{sessionsExpanded ? (
									...
									<ChevronRight className={sizeClasses.iconSize} />
								)}
							</button>
						)}
						<span
			...
			{sessionsExpanded && (
				<div
					className={cn(
						externalScroll
							? "space-y-1 px-1"
							: "flex-1 min-h-0 overflow-y-auto overflow-x-hidden space-y-1 px-1",
					)}
				>
											...
											<div className="space-y-0.5 pb-1">
												{projectSessionsForMode
													...
													.map((session) => {
														...
														const hasChildren = children.length > 0;
														const forkDescendants = flattenForkDescendants(
															sessionHierarchy.childSessionsByParent,
															session.id,
														);
														const isExpanded = expandedSessions.has(session.id);
