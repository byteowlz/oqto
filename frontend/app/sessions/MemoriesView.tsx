...
// API functions
async function fetchMemories(sessionId: string, offset = 0, limit = 50): Promise<MemoryListResponse> {
  const baseUrl = controlPlaneDirectBaseUrl()
  const url = new URL(`${baseUrl}/session/${sessionId}/memories`)
...
async function searchMemories(sessionId: string, query: string, limit = 50): Promise<Memory[]> {
  const baseUrl = controlPlaneDirectBaseUrl()
  const res = await fetch(`${baseUrl}/session/${sessionId}/memories/search`, {
    method: "POST",
    credentials: "include",
...
async function addMemory(
  ...
  const baseUrl = controlPlaneDirectBaseUrl()
  const res = await fetch(`${baseUrl}/session/${sessionId}/memories`, {
    method: "POST",
    credentials: "include",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ 
...
async function deleteMemory(sessionId: string, memoryId: string): Promise<void> {
  const baseUrl = controlPlaneDirectBaseUrl()
  const res = await fetch(`${baseUrl}/session/${sessionId}/memories/${memoryId}`, {
    method: "DELETE",
    credentials: "include",
  })
  if (!res.ok) {
...
export function MemoriesView({ className }: MemoriesViewProps) {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      const results = await searchMemories(selectedWorkspaceSessionId, searchQuery)
      setMemories(results)
  ...
  const handleAdd = useCallback(async () => {
    ...
    setError("")
    try {
      const newMemory = await addMemory(selectedWorkspaceSessionId, newMemoryContent)
  ...
  const handleEdit = useCallback(async (memoryId: string, content: string) => {
    if (!selectedWorkspaceSessionId) return

    try {
      const updated = await updateMemory(selectedWorkspaceSessionId, memoryId, content)
      ...
      <div className="flex-shrink-0 p-2 border-b border-border space-y-2">
        ...
        {!loading && (
          <div className="text-xs text-muted-foreground">
            {isSearchMode ? (
              <span>Found {memories.length} result{memories.length !== 1 ? "s" : ""} for "{searchQuery}"</span>
            ) : (
