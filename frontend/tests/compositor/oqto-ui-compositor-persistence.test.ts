...
import { decodeLayoutDocument, encodeLayoutDocument, recoverLayoutDocument } from "@/src/oqto-ui/compositor/kernel/persistence/codec";
import { runLayoutMigrations } from "@/src/oqto-ui/compositor/kernel/persistence/migrations";
import { describe, expect, it } from "vitest";
import { chatContent, classicLayout, filesContent, sessionsContent } from "./fixtures";
...
describe("versioned layout persistence (schema v2)", () => {
	...
	it("preserves unknown variants and extension fields at every level", () => {
		const raw = mutated((doc) => {
			...
			(containers[1].stack as { [key: string]: unknown }[])[0].futureContentField = true;
			(containers[2].stack as { [key: string]: unknown }[])[0].kind = "kind-from-the-future";
		});
		const decoded = decodeLayoutDocument(raw);
		expect(decoded.ok).toBe(true);
		if (!decoded.ok) return;
		expect(decoded.snapshot.extensions).toEqual({ futureTopLevel: { anything: [1, 2, 3] } });
		expect(decoded.snapshot.arrangements[0].extensions).toEqual({ futureArrangementField: "kept" });
		expect(decoded.snapshot.containers[0].extensions).toEqual({ futureContainerField: "kept" });
		expect(decoded.snapshot.containers[1].stack[0].extensions).toEqual({ futureContentField: true });
		expect(decoded.snapshot.containers[2].stack[0].kind).toBe("kind-from-the-future");
		expect(JSON.parse(encodeLayoutDocument(decoded.snapshot))).toEqual(JSON.parse(raw));
	});

	it.each([
		{ name: "truncated JSON", raw: '{"schemaVersion": 2, "rev', reason: "parse-error" },
		{ name: "non-object root", raw: "[1,2,3]", reason: "invalid-shape" },
		{ name: "missing schemaVersion", raw: mutated((doc) => { doc.schemaVersion = undefined; }), reason: "invalid-shape" },
		{ name: "newer schemaVersion", raw: mutated((doc) => { doc.schemaVersion = 99; }), reason: "unsupported-version" },
		{ name: "unknown empty behavior", raw: mutated((doc) => { (doc.containers as { [k: string]: unknown }[])[0].emptyBehavior = "detonate"; }), reason: "invalid-shape" },
		{ name: "malformed active workspace", raw: mutated((doc) => { doc.activeWorkspace = { kind: "galaxy" }; }), reason: "invalid-shape" },
		{
			name: "negative track size",
			raw: mutated((doc) => {
				(doc.arrangements as { grid: { columns: { size: { value: number } }[] } }[])[0].grid.columns[0].size.value = -5;
			}),
		...
		},
		{
			...
			raw: mutated((doc) => {
				(doc.arrangements as { grid: { placements: { column: number }[] } }[])[0].grid.placements[1].column = 0;
			}),
			reason: "invariant-violation",
		},
		{ name: "active content missing from its stack", raw: mutated((doc) => { (doc.containers as { [k: string]: unknown }[])[0].activeContentId = "nonexistent"; }), reason: "invariant-violation" },
		{ name: "focus on unplaced content", raw: mutated((doc) => { doc.focusedContentId = "nonexistent"; }), reason: "invariant-violation" },
		{ name: "unknown active arrangement", raw: mutated((doc) => { doc.activeArrangementId = "arrangement-404"; }), reason: "invariant-violation" },
	...
	it("migrates a schema v1 document into one Arrangement with spans", () => {
		...
		expect(grid.columns.map((c) => c.id)).toEqual(["column-1", "column-3", "column-5"]);
		const nav = grid.placements.find((p) => p.containerId === "container-0");
		expect(nav).toMatchObject({ row: 0, column: 0, rowSpan: 2, colSpan: 1 });
		expect(grid.placements.find((p) => p.containerId === "container-2")).toMatchObject({ row: 0, rowSpan: 1, column: 1 });
		expect(grid.placements.find((p) => p.containerId === "container-6")).toMatchObject({ row: 1, rowSpan: 1, column: 1 });
		...
		expect(encodeLayoutDocument(snapshot)).toBe(encodeLayoutDocument(decodeLayoutDocument(encodeLayoutDocument(snapshot)).ok ? snapshot : snapshot));
	});
	...
	it("walks a migration chain in order and fails closed on a missing step", () => {
		...
		const result = runLayoutMigrations(
			...
			[
				{ from: 3, migrate: (doc) => { applied.push(3); return { ...doc, addedInV4: true }; } },
				{ from: 2, migrate: (doc) => { applied.push(2); return { ...doc, addedInV3: true }; } },
			],
			4,
		);
		...
		expect(runLayoutMigrations({ ...classicDocument(), schemaVersion: 2 }, [{ from: 2, migrate: (doc) => doc }], 4)).toBeNull();
	});

	it("recovers the first healthy candidate and falls back when all are corrupt", () => {
		const layout = classicLayout();
		const recovered = recoverLayoutDocument(["{corrupt", encodeLayoutDocument(layout)], classicLayout());
		expect(recovered.sourceIndex).toBe(1);
		expect(recovered.snapshot).toEqual(layout);
		expect(recovered.failures).toEqual([{ index: 0, reason: "parse-error", detail: expect.stringContaining("JSON") }]);
		const fallback = classicLayout();
		const none = recoverLayoutDocument([null, undefined, '{"schemaVersion": 99}'], fallback);
