...
import { decodeLayoutDocument, encodeLayoutDocument } from "@/src/oqto-ui/compositor/kernel/persistence/codec";
...
interface Step {
	command: LayoutCommand | { type: "serialize-restore" } | { type: "stale" } | { type: "project" };
...
function placedIds(snapshot: LayoutSnapshot): ContentId[] {
	return snapshot.containers.flatMap((container) => container.stack.map((content) => content.id));
}
...
function activeGrid(snapshot: LayoutSnapshot) {
	return snapshot.arrangements.find((a) => a.id === snapshot.activeArrangementId)?.grid;
}
...
function generateStep(rng: SeededRng, model: Model): Step {
	...
	if (roll < 12) {
		const content = { id: contentIdFrom(`generated-${model.minted}`), kind: rng.pick(KINDS) };
		const r = rng.int(4);
		const target =
			r === 0 && containers.length > 0
				...
				: r === 1
					...
					: r === 2
						...
						: undefined;
		return { command: { type: "open", content, ...(target ? { target } : {}) }, expectation: "accept" };
	}
	if (roll < 17 && placed.length > 0) {
		...
		return { command: { type: "open", content: { id: existing.id, kind: existing.kind } }, expectation: "accept" };
	}
	if (roll < 22) {
		const known = placed.length > 0 && rng.bool(0.7);
		return {
			command: { type: "reveal", contentId: known ? rng.pick(placed) : contentIdFrom(`ghost-${roll}`) },
			expectation: known ? "accept" : "reject",
		};
	}
	if (roll < 27 && placed.length > 0) {
		return { command: { type: rng.bool() ? "activate" : "focus", contentId: rng.pick(placed) }, expectation: "accept" };
	}
	if (roll < 35 && placed.length > 0) {
		const destination = rng.bool(0.7)
			? { containerId: rng.pick(containers), position: rng.int(5) }
			: { arrangementId: rng.pick(arrangements) };
		return { command: { type: "move", contentId: rng.pick(placed), destination }, expectation: "accept" };
	}
	if (roll < 43 && placed.length > 0) {
		const edge = rng.pick(EDGES);
		const relative = rng.bool(0.6) && containers.length > 0;
		return {
			command: { type: "split", contentId: rng.pick(placed), edge, ...(relative ? { relativeTo: rng.pick(containers) } : {}) },
			expectation: "either",
		};
	}
	if (roll < 49 && containers.length > 0) {
		...
		return {
			command: {
				...
				size: valid ? { unit: rng.bool() ? "fraction" : "fixed", value: 1 + rng.int(500) } : { unit: "fraction", value: -rng.int(3) },
			},
			expectation: valid ? "either" : "reject",
		};
	}
	if (roll < 54 && containers.length > 0) {
		return { command: { type: "collapse", containerId: rng.pick(containers), collapsed: rng.bool() }, expectation: "accept" };
	}
	if (roll < 58) {
		const grid = activeGrid(s);
		if (grid && grid.rows.length > 0) {
			return { command: { type: "flush", rowId: rng.pick(grid.rows).id, flush: rng.bool(0.7) }, expectation: "either" };
		}
	}
	if (roll < 62) {
		const grid = activeGrid(s);
		const anchor = grid && grid.columns.length > 0 && rng.bool(0.8) ? rng.pick(grid.columns).id : null;
		return { command: { type: "scroll", anchorColumnId: anchor }, expectation: "accept" };
	}
	if (roll < 72 && placed.length > 0) {
		return { command: { type: "close", contentId: rng.pick(placed) }, expectation: "accept" };
	}
	if (roll < 75) {
		return {
			command: { type: "arrangement-create", ...(rng.bool() ? { binding: { kind: "work-directory", id: `wd-${roll}` } } : {}), activate: rng.bool() },
			expectation: "accept",
		};
	}
	if (roll < 79) {
		return { command: { type: "arrangement-switch", arrangementId: rng.pick(arrangements) }, expectation: "accept" };
	}
	if (roll < 82) {
		return {
	...
	if (roll < 84) {
		return {
			command: { type: "workspace-switch", workspace: rng.bool() ? { kind: "all" } : { kind: "workspace", id: `ws-${roll}` } },
			expectation: "accept",
		};
	}
	if (roll < 88 && model.history.length > 0) {
		return { command: { type: "undo", snapshot: rng.pick(model.history) }, expectation: "accept" };
	}
	if (roll < 93) return { command: { type: "serialize-restore" }, expectation: "accept" };
...
function applyStep(model: Model, step: Step): Model {
	...
	if (step.command.type === "serialize-restore") {
		...
		expect(encodeLayoutDocument(decoded.snapshot)).toBe(encodeLayoutDocument(before));
	...
	if (step.command.type === "project") {
		...
		for (const inlineSize of [1600, 1100, 700, 420]) {
			const projected = projectLayout(before, { inlineSize, blockSize: 800, responsive: "merge" });
			expect(checkLayoutInvariants(projected.snapshot)).toEqual([]);
			expect([...placedIds(projected.snapshot)].sort()).toEqual([...placedIds(before)].sort());
			expect(projected.geometry.degradations.some((d) => d.kind === "overflow")).toBe(false);
			expect(projectLayout(before, { inlineSize, blockSize: 800, responsive: "merge" })).toEqual(projected);
		}
		expect(encodeLayoutDocument(before)).toBe(encodedBefore);
		return model;
	}
	if (step.command.type === "stale") {
		const result = applyTransaction(before, { expectedRevision: before.revision + 1 + (before.revision % 3), commands: [] });
	...
	const result = applyTransaction(before, { expectedRevision: before.revision, commands: [command], viewport: VIEWPORT });

	if (!result.ok) {
		if (step.expectation === "accept") throw new Error(`expected acceptance, got ${JSON.stringify(result.rejection)}`);
		...
		expect(JSON.parse(JSON.stringify(result.rejection))).toEqual(result.rejection);
		return model;
	}
	if (step.expectation === "reject") throw new Error(`expected rejection for ${JSON.stringify(command)}`);

	const after = result.snapshot;
	const violations = checkLayoutInvariants(after);
	if (violations.length > 0) throw new Error(`invariants violated: ${JSON.stringify(violations)}`);
	...
	if (["move", "split", "reveal", "activate", "focus", "collapse", "resize", "flush", "scroll", "arrangement-create", "arrangement-switch", "workspace-switch"].includes(command.type)) {
	...
	if (command.type === "arrangement-remove") {
		expect(after.arrangements.some((a) => a.id === command.arrangementId)).toBe(false);
	}
	if (command.type === "reveal") {
		const container = after.containers.find((c) => c.stack.some((content) => content.id === command.contentId));
		expect(container?.activeContentId).toBe(command.contentId);
		expect(container?.collapsed).toBe(false);
		const owner = after.arrangements.find((a) => a.grid.placements.some((p) => p.containerId === container?.id));
		expect(owner?.id as ArrangementId).toBe(after.activeArrangementId);
	}

	deepFreeze(after);
	const minted = command.type === "open" && command.content.id.startsWith("generated-") ? model.minted + 1 : model.minted;
	return { snapshot: after, history: [...model.history.slice(-9), before], minted };
}
...
describe("compositor model properties", () => {
	it("preserves every invariant across generated command sequences", () => {
		runGeneratedSequences<Model, Step>({
			...
			stepsPerSeed: 80,
			initialModel: () => ({ snapshot: deepFreeze(classicLayout()), history: [], minted: 0 }),
			generateStep,
