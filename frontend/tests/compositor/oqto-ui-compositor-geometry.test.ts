...
function assertSane(rects: readonly SolvedRect[]) {
	...
	for (const a of visible) {
		for (const b of visible) {
			if (a === b) continue;
			const inlineOverlap =
				a.x < b.x + b.inlineSize && b.x < a.x + a.inlineSize;
			const blockOverlap = a.y < b.y + b.blockSize && b.y < a.y + a.blockSize;
			expect(
				inlineOverlap && blockOverlap,
				`${a.containerId} overlaps ${b.containerId}`,
			).toBe(false);
		}
	}
}
...
describe("semantic grid geometry (v2)", () => {
	it("solves the classic preset: fixed nav, 2:1 primary/auxiliary, full height", () => {
		...
		const { rects, degradations, columnSizes } = solveLayoutGeometry(
			layout,
			VIEWPORT,
		);
	...
	it("respects host safe areas", () => {
		const layout = classicLayout();
		const { rects } = solveLayoutGeometry(layout, {
			...VIEWPORT,
	...
	it("collapse preserves the Container without consuming its track", () => {
		...
		expect(rectOf(rects, navId)).toMatchObject({
			collapsed: true,
			inlineSize: 0,
		});
	...
	it("a flush bottom Lane spans the screen width, is pinned, and ignores scrolling", () => {
		...
		const laneContainer = activeArrangementOf(layout).grid.placements.find(
			(p) => p.row === 1,
		...
		layout = step(layout, {
			...
			axis: "inline",
			size: { unit: "fixed", value: 2000 },
		});
		layout = step(layout, {
			type: "scroll",
			anchorColumnId: activeArrangementOf(layout).grid.columns[2].id,
		});
		...
		expect(
			rectOf(solved.rects, containerByRole(layout, "navigation").id).x,
		).toBeLessThan(0);
	});

	it("scroll policy honors minima and reports overflow instead of scaling", () => {
		const layout = classicLayout();
		const scroll = solveLayoutGeometry(layout, {
			inlineSize: 500,
			blockSize: 400,
		});
		expect(scroll.degradations).toContainEqual({
			kind: "overflow",
			axis: "inline",
			extent: expect.any(Number),
		});
		expect(scroll.inlineExtent).toBeGreaterThan(500);
		expect(scroll.columnSizes[0]).toBe(320);
		const fit = solveLayoutGeometry(layout, {
			inlineSize: 500,
			blockSize: 400,
			overflow: "fit",
		});
		expect(fit.degradations).toContainEqual({
			kind: "minima-unsatisfiable",
			axis: "inline",
			deficit: expect.any(Number),
		});
		expect(fit.columnSizes.reduce((a, b) => a + b, 0)).toBeLessThanOrEqual(
			500 + 0.001,
		);
	...
	it("derives geometry purely and independently of focus", () => {
		...
		expect(solveLayoutGeometry(focused, VIEWPORT)).toEqual(
	...
	it("scroll anchors beyond the extent clamp to the last screenful", () => {
		...
		layout = step(layout, {
			type: "open",
			content: { id: contentIdFrom("x"), kind: "chat" },
		});
		layout = step(layout, {
			type: "split",
			contentId: contentIdFrom("x"),
			edge: "inline-end",
		});
