...
export const workspaceSessionKeys = {
  ...
  list: (filters: Record<string, unknown>) => [...workspaceSessionKeys.lists(), filters] as const,
...
export function useCreateWorkspaceSession() {
  ...
  return useMutation({
    mutationFn: (request: CreateWorkspaceSessionRequest) => createWorkspaceSession(request),
    onSuccess: (newSession) => {
      // Update the sessions list cache
      queryClient.setQueryData<WorkspaceSession[]>(workspaceSessionKeys.lists(), (old) => {
...
export function useStopWorkspaceSession() {
  ...
  return useMutation({
    ...
    onMutate: async (sessionId) => {
      ...
      await queryClient.cancelQueries({ queryKey: workspaceSessionKeys.lists() })

      // Snapshot the previous value
      const previousSessions = queryClient.getQueryData<WorkspaceSession[]>(workspaceSessionKeys.lists())

      // Optimistically update to the stopped status
      queryClient.setQueryData<WorkspaceSession[]>(workspaceSessionKeys.lists(), (old) => {
        if (!old) return old
        return old.map((session) =>
          session.id === sessionId ? { ...session, status: "stopping" as const } : session,
        )
      })
    ...
    onError: (_err, _sessionId, context) => {
      ...
      if (context?.previousSessions) {
        queryClient.setQueryData(workspaceSessionKeys.lists(), context.previousSessions)
      }
    },
    onSettled: () => {
