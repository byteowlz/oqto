...
import type { VoiceConfig, VoiceState, VoiceSettings, VisualizerType, VisualizerVoiceSettings } from '@/lib/voice/types';
import { DEFAULT_VOICE_SETTINGS, DEFAULT_VISUALIZER_VOICES } from '@/lib/voice/types';
...
export function useVoiceMode(options: UseVoiceModeOptions): UseVoiceModeReturn {
  ...
  const [visualizerVoices, setVisualizerVoices] = useState<Record<string, VisualizerVoiceSettings>>(loadVisualizerVoices);
    ...
    if (config) {
      ...
      if (config.visualizer_voices) {
        setVisualizerVoices(_prev => {
          ...
          for (const [vizId, vizConfig] of Object.entries(config.visualizer_voices)) {
            fromConfig[vizId] = { voice: vizConfig.voice, speed: vizConfig.speed };
          }
  ...
  useEffect(() => {
    ...
    if (vizVoice && ttsRef.current?.isConnected()) {
      console.log('[Voice] Switching to visualizer voice:', settings.visualizer, vizVoice);
      
      // Stop current playback and capture any pending text
      const pendingTexts = ttsRef.current.stopPlayback();
      
      // Apply new voice and speed
      (async () => {
        try {
          await ttsRef.current!.setVoice(vizVoice.voice);
          await ttsRef.current!.setSpeed(vizVoice.speed);
          
          // Re-queue pending text with new voice
          if (pendingTexts.length > 0) {
            console.log('[Voice] Re-queuing', pendingTexts.length, 'pending texts with new voice');
            for (const text of pendingTexts) {
              ttsRef.current!.speak(text).catch(console.error);
            }
          }
        } catch (err) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      smoothInputRef.current += (rawInput - smoothInputRef.current) * smoothingFactor;
      smoothOutputRef.current += (rawOutput - smoothOutputRef.current) * smoothingFactor;
    ...
    if (!sttRef.current) {
      ...
      sttRef.current.setCallbacks({
        onWord: (word) => {
          ...
          if (voiceStateRef.current === 'speaking' && settingsRef.current.interruptWordCount > 0) {
            interruptWordCountRef.current++;
            console.log('[Voice] Word during TTS:', word, 'count:', interruptWordCountRef.current);
            ...
            if (interruptWordCountRef.current >= settingsRef.current.interruptWordCount) {
  ...
  const setInterruptWordCount = useCallback((count: number) => {
    setSettings(prev => ({ ...prev, interruptWordCount: Math.max(0, count) }));
  }, []);
