...
import { getAuthToken, getControlPlaneBaseUrl } from "@/lib/control-plane-client";
...
export type PromptType = "file_read" | "file_write" | "ssh_sign" | "network_access" | string;
...
export function usePrompts(options: UsePromptsOptions = {}): UsePromptsReturn {
    ...
    try {
      ...
      ws.onclose = (event) => {
        ...
        if (event.code !== 1000) { // Not a clean close
          ...
          console.log(`[prompts] Reconnecting in ${delay}ms (attempt ${reconnectAttemptRef.current})`);
  ...
  // Respond to a prompt
  const respond = useCallback(async (promptId: string, action: PromptAction) => {
