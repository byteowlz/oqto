...
export class TTSService {
  ...
  private currentVoice: string = 'af_heart';
  ...
  private handleMessage(message: TTSMessage) {
    switch (message.type) {
      case 'voices':
        ...
        this.callbacks.onVoicesLoaded?.(this.availableVoices, this.currentVoice);
        break;
  ...
  private async handleAudioChunk(base64Chunk: string) {
    ...
    try {
      ...
      if (!this.audioContext) {
        ...
        console.log('[TTS] AudioContext created, sample rate:', this.audioContext.sampleRate);
  ...
  async setSpeed(speed: number): Promise<void> {
    ...
    this.ws.send(JSON.stringify({ command: 'set_speed', speed: this.currentSpeed }));
  }
  ...
  setAutoDetect(enabled: boolean) {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      this.ws.send(JSON.stringify({ command: 'set_auto_detect', auto_detect: enabled }));
    }
  }
