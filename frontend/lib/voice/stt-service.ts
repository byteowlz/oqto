...
export class STTService {
  ...
  constructor(
    ...
    vadSilenceTimeoutMs: number = 1500
  ) {
  ...
  private handleMessage(message: STTMessage) {
    switch (message.type) {
      case 'word':
        if (message.word) {
          this.currentTranscript += (this.currentTranscript ? ' ' : '') + message.word;
  ...
  async startListening(): Promise<void> {
    ...
    if (!window.isSecureContext) {
      throw new Error('Voice mode requires HTTPS. Please access the site via https:// or localhost');
    }

    // Check for mediaDevices API
    if (!navigator.mediaDevices?.getUserMedia) {
      throw new Error('getUserMedia not supported. Voice mode requires a modern browser with HTTPS');
    }
