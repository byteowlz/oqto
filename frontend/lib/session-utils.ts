...
export function parseAutoGeneratedTitle(title: string): ParsedPiTitle {
	...
	const pattern = /^(?<workspace>[^:]+):\s*(?<title>[^\[]+)\s*\[(?<readable_id>[^\]]+)\]\s*$/;
	const match = trimmed.match(pattern);

	if (match && match.groups) {
		return {
			workspace: match.groups?.workspace?.trim() || null,
			title: match.groups?.title?.trim() || trimmed,
			readable_id: match.groups?.readable_id?.trim() || null,
		};
	}
...
export function getDisplayPiTitle(session: {
	...
	return parsed.title || session.title;
}
