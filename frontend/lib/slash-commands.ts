...
export function commandInfoToSlashCommands(commands: OpenCodeCommandInfo[]): SlashCommand[] {
  return commands.map(cmd => ({
    name: cmd.name,
    description: cmd.description || cmd.template.slice(0, 60) + (cmd.template.length > 60 ? "..." : ""),
...
export function filterCommands(commands: SlashCommand[], query: string): SlashCommand[] {
  ...
  return commands
    .filter(cmd => fuzzyMatch(query, cmd.name) || fuzzyMatch(query, cmd.description))
    .sort((a, b) => {
...
export function parseSlashInput(text: string): { isSlash: boolean; command: string; args: string } {
  if (!text.startsWith("/")) {
