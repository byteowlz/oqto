		...
		<div className="flex flex-col gap-4 h-full min-h-0 p-4 md:p-6 overflow-y-auto w-full">
			...
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 w-full">
				{[
					...
					<div
						key={label}
						className="bg-card border border-border p-3 md:p-4 flex flex-col gap-2 md:gap-3 hover:border-primary transition"
					>
						<div className="flex items-center justify-between">
							...
							<Icon
								className={`w-6 h-6 md:w-8 md:h-8 shrink-0 ${idx === 2 || idx === 3 ? "text-foreground" : "text-primary"}`}
							/>
						</div>
							...
							{activeSessions.map((session, index) => (
								...
								>
									...
									<td className="py-3 px-4">
										<span className="text-xs px-2 py-1 bg-primary/20 text-primary">
											{session.template}
										</span>
									</td>
