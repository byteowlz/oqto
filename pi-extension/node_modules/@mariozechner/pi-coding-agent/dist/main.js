...
import { discoverAndLoadExtensions } from "./core/extensions/index.js";
...
async function runInteractiveMode(session, version, changelogMarkdown, modelFallbackMessage, modelsJsonError, migratedProviders, versionCheckPromise, initialMessages, extensions, setExtensionUIContext, initialMessage, initialImages, fdPath = undefined) {
...
function discoverSystemPromptFile() {
...
function buildSessionOptions(parsed, scopedModels, sessionManager, modelRegistry, preloadedExtensions) {
    const options = {};
...
export async function main(args) {
    ...
    if (mode === "rpc") {
        ...
        time("ensureTool(fd)");
        printTimings();
        await runInteractiveMode(session, VERSION, changelogMarkdown, modelFallbackMessage, modelRegistry.getError(), migratedProviders, versionCheckPromise, parsed.messages, extensionsResult.extensions, extensionsResult.setUIContext, initialMessage, initialImages, fdPath);
    }
    else {
