...
./scripts/e2e/convert-setup-toml.sh oqto.setup.toml >> scripts/vm.tests.toml
```
...
test-vm-deployment:
  ...
  steps:
    ...
    - name: Run VM Tests
      ...
      run: |
        cp scripts/e2e/vm.tests.toml.example scripts/vm.tests.toml
        ./scripts/e2e/test-vm-deployment.sh --scenario ubuntu-24-04-local-single
```
