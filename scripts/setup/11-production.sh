...
setup_caddy_prompt() {
  ...
  echo
  echo "Without Caddy: access locally via http://<LAN-IP>:8080 or http://localhost:8080"
  echo
...
setup_caddy_config() {
  ...
  if [[ "$DOMAIN" == "localhost" ]]; then
    ...
    if [[ -z "${OQTO_CADDY_PORT:-}" ]]; then
      OQTO_CADDY_PORT="$(prompt_input "Caddy listen port" "8086")"
    fi
    log_info "Caddy will listen on port: ${OQTO_CADDY_PORT}"
  else
    log_info "Caddy will obtain TLS certificate for: $DOMAIN"
    if [[ "${OQTO_CADDY_PORT:-80}" != "80" ]]; then
      log_warn "Ignoring OQTO_CADDY_PORT=${OQTO_CADDY_PORT} for TLS domain setup (Caddy uses 80/443)."
    fi
  fi
}
...
generate_caddyfile() {
  ...
  if [[ "$DOMAIN" == "localhost" ]]; then
    ...
    local caddy_port="${OQTO_CADDY_PORT:-8086}"
