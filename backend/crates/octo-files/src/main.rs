...
struct Cli {
    /// Port to listen on
    #[arg(short, long, env = "OCTO_FILES_PORT", default_value = "41821")]
    port: u16,

    /// Address to bind to
    #[arg(short, long, env = "OCTO_FILES_BIND", default_value = "0.0.0.0")]
    bind: String,

    /// Root directory to serve files from
    #[arg(short, long, env = "OCTO_FILES_ROOT", default_value = ".")]
    root: PathBuf,

    /// Enable verbose logging
    #[arg(short, long, env = "OCTO_FILES_VERBOSE")]
    verbose: bool,

    /// Config file path (optional)
    #[arg(short, long, env = "OCTO_FILES_CONFIG")]
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let filter = if cli.verbose {
        "octo_files=debug,tower_http=debug"
    } else {
        "octo_files=info,tower_http=info"
    };
    ...
    info!("Starting octo-files on {}", addr);
