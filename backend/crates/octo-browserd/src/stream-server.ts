...
export class StreamServer {
      ...
      try {
        this.wss = new WebSocketServer({
          ...
          verifyClient: (info: {
            ...
            console.log(`[octo-browserd] Rejected connection from origin: ${info.origin}`);
            return false;
          },
        });
        ...
        this.wss.on("error", (error) => {
          console.error("[octo-browserd] stream server error:", error);
          reject(error);
        });
        this.wss.on("listening", () => {
          console.log(`[octo-browserd] Stream server listening on port ${this.port}`);
          resolve();
        });
      } catch (error) {
        reject(error);
  ...
  private handleConnection(ws: WebSocket): void {
    console.log("[octo-browserd] Client connected");
    this.clients.add(ws);
    this.sendStatus(ws);

    if (this.clients.size === 1 && !this.isScreencasting) {
      this.startScreencast().catch((error) => {
        console.error("[octo-browserd] Failed to start screencast:", error);
    ...
    ws.on("close", () => {
      console.log("[octo-browserd] Client disconnected");
    ...
    ws.on("error", (error) => {
      console.error("[octo-browserd] Client error:", error);
      this.clients.delete(ws);
    });
  }
