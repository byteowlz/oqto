// Seccomp BPF compilation depends on Linux libseccomp. Keep the CLI buildable
// on macOS, but never pretend to generate policy artifacts there.
#[cfg(target_os = "linux")]
mod linux {
...
fn main() -> anyhow::Result<()> {
    anyhow::bail!("seccomp-policy-gen requires Linux and libseccomp")
}
