# Session: octo-browser socket connection error
...
## Summary: octo-browser Architecture

### Overview

`octo-browser` is a **client-only CLI** that communicates with an **external daemon** called `agent-browser`. It is NOT the daemon itself - it's a lightweight Rust CLI wrapper for talking to the daemon over Unix sockets (or TCP on Windows).

### Key Components

| Component | Location | Role |
|-----------|----------|------|
| `octo-browser` | `/home/wismut/byteowlz/octo/backend/crates/octo-browser` | Rust CLI client for Octo's integration |
...
1. **The daemon is part of `agent-browser`** (external project), not `octo-browser`
...
3. **Octo backend integration**: The `AgentBrowserManager` in `/home/wismut/byteowlz/octo/backend/crates/octo/src/agent_browser.rs` starts the daemon by calling:
   ```
...
```
+-------------------+          Unix Socket              +---------------------+
|   octo-browser    |  ------>  /tmp/agent-browser-    |   agent-browser     |
...
**Option 3: Via Octo backend (production)**
...
| Binary | Purpose |
|--------|---------|
| `octo-browser` | Octo's Rust CLI client (this crate) |
| `agent-browser` | External CLI + daemon (Vercel project) |

The `octo-browser` crate is essentially a **thin wrapper** that mirrors `agent-browser`'s protocol but is built in Rust for consistency with the Octo backend. Both communicate with the same daemon using the same JSON protocol over Unix sockets.
...
**`octo-browser` is a client-only CLI** that connects to an external daemon called `agent-browser`. The daemon isn't running, which is why you're getting the "No such file or directory" error.
...
# Then octo-browser can connect to the same session
octo-browser open https://example.com
```
...
| `octo-browser` | Rust CLI client that talks to the daemon via Unix socket |

The socket `/tmp/agent-browser-default.sock` is created by the Node.js daemon when it starts, not by `octo-browser`.
