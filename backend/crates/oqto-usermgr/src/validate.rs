//! Input validation for oqto-usermgr.
//!
...
pub const USERNAME_PREFIX: &str = "oqto_";

/// Required group name.
pub const REQUIRED_GROUP: &str = "oqto";
...
pub const GECOS_PREFIX: &str = "Oqto platform user ";
...
/// Validate the group name. Must be exactly "oqto".
...
///
/// Must start with "Oqto platform user ", no dangerous characters.
...
mod tests {
    ...
    fn username_valid_simple() {
        assert!(validate_username("oqto_admin").is_ok());
    }
    ...
    fn username_valid_with_nanoid_suffix() {
        assert!(validate_username("oqto_admin-a1b2").is_ok());
    }
    ...
    fn username_reject_just_prefix() {
        assert!(validate_username("oqto_").is_err());
    }
    ...
    fn group_reject_other() {
        ...
        assert!(validate_group("oqto_").is_err());
        assert!(validate_group("Oqto").is_err());
    }
    ...
    fn path_valid_home() {
        let prefixes = &["/run/oqto/runner-sockets/", "/home/oqto_"];
        assert!(validate_path("/home/oqto_admin", prefixes).is_ok());
        assert!(validate_path("/home/oqto_admin/workspace", prefixes).is_ok());
    }

    #[test]
    fn path_reject_traversal() {
        let prefixes = &["/run/oqto/runner-sockets/", "/home/oqto_"];
        assert!(validate_path("/home/oqto_admin/../root", prefixes).is_err());
        assert!(validate_path("/run/oqto/runner-sockets/../../../etc/passwd", prefixes).is_err());
        assert!(validate_path("/home/oqto_admin/..%2f..%2fetc", prefixes).is_err()); // encoded .. still contains ..
    }

    #[test]
    fn path_reject_double_slash() {
        let prefixes = &["/run/oqto/runner-sockets/", "/home/oqto_"];
        assert!(validate_path("/home/oqto_admin//evil", prefixes).is_err());
    ...
    fn path_reject_relative() {
        let prefixes = &["/run/oqto/runner-sockets/", "/home/oqto_"];
        assert!(validate_path("home/oqto_admin", prefixes).is_err());
        assert!(validate_path("./home/oqto_admin", prefixes).is_err());
        assert!(validate_path("oqto_admin", prefixes).is_err());
    }
    ...
    fn path_reject_wrong_prefix() {
        ...
        assert!(validate_path("/var/lib/oqto", prefixes).is_err());
    }

    #[test]
    fn path_reject_null_byte() {
        let prefixes = &["/run/oqto/runner-sockets/", "/home/oqto_"];
        assert!(validate_path("/home/oqto_admin\0/etc/passwd", prefixes).is_err());
    }

    #[test]
    fn path_reject_control_chars() {
        let prefixes = &["/run/oqto/runner-sockets/", "/home/oqto_"];
        assert!(validate_path("/home/oqto_admin\n/etc/passwd", prefixes).is_err());
        assert!(validate_path("/home/oqto_admin\t/evil", prefixes).is_err());
    }
    ...
    fn path_reject_symlink_trickery() {
        ...
        let prefixes = &["/run/oqto/runner-sockets/", "/home/oqto_"];
        assert!(validate_path("/home/oqto_admin/../../etc/shadow", prefixes).is_err());
    }
    ...
    #[test]
    fn gecos_valid() {
        assert!(validate_gecos("Oqto platform user admin-a1b2").is_ok());
        assert!(validate_gecos("Oqto platform user hansgerd-x9z1").is_ok());
    }
    ...
    fn gecos_reject_colon() {
        ...
        assert!(validate_gecos("Oqto platform user admin:0:0:root").is_err());
    }

    #[test]
    fn gecos_reject_newline() {
        assert!(validate_gecos("Oqto platform user admin\nevil:0:0::/root:/bin/bash").is_err());
    }

    #[test]
    fn gecos_reject_null() {
        assert!(validate_gecos("Oqto platform user admin\0evil").is_err());
    }

    #[test]
    fn gecos_reject_carriage_return() {
        assert!(validate_gecos("Oqto platform user admin\revil").is_err());
    }

    #[test]
    fn gecos_reject_too_long() {
        let long = format!("Oqto platform user {}", "a".repeat(GECOS_MAX_LEN));
    ...
    fn owner_reject_wrong_format() {
        assert!(validate_owner("oqto_admin").is_err()); // no colon
        assert!(validate_owner("oqto_admin:oqto:extra").is_err()); // too many colons
        assert!(validate_owner(":oqto").is_err()); // empty user
        assert!(validate_owner("oqto_admin:").is_err()); // empty group
    }

    #[test]
    fn owner_reject_wrong_user() {
        assert!(validate_owner("root:oqto").is_err());
        assert!(validate_owner("tommy:oqto").is_err());
    }

    #[test]
    fn owner_reject_wrong_group() {
        assert!(validate_owner("oqto_admin:root").is_err());
        assert!(validate_owner("oqto_admin:wheel").is_err());
    }
    ...
    fn injection_gecos_passwd_line() {
        ...
        assert!(validate_gecos("Oqto platform user x\nevil:0:0::/root:/bin/bash").is_err());
    }

    #[test]
    fn injection_gecos_field_escape() {
        // Attempt to break out of GECOS field via colon
        assert!(validate_gecos("Oqto platform user x:0:0:root:/root:/bin/bash").is_err());
    }
    ...
    fn injection_path_escape_home() {
        ...
        assert!(validate_path("/home/oqto_evil/../../../etc/shadow", prefixes).is_err());
        assert!(validate_path("/home/oqto_evil/../../root/.ssh", prefixes).is_err());
    }
    ...
    fn injection_path_null_truncation() {
        ...
        assert!(validate_path("/home/oqto_admin\0/../../../etc/shadow", prefixes).is_err());
    }
}
