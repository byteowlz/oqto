...
#[derive(Parser)]
#[command(name = "oqto-browser", version, about = "CLI for agent-browser daemon")]
struct Cli {
...
/// Resolve the oqto-browserd lib directory.
///
/// Checks (in order):
///   1. /usr/local/lib/oqto-browserd  (system install)
///   2. <repo>/backend/crates/oqto-browserd  (dev checkout, relative to this binary)
///   3. OQTO_BROWSERD_DIR env var
fn find_browserd_dir() -> Result<PathBuf> {
    // Env override
    if let Ok(dir) = std::env::var("OQTO_BROWSERD_DIR") {
    ...
    // System install
    let system = PathBuf::from("/usr/local/lib/oqto-browserd");
    if system.join("node_modules").exists() {
        return Ok(system);
    }

    // Dev checkout: walk up from the binary location looking for backend/crates/oqto-browserd
    if let Ok(exe) = std::env::current_exe() {
        let mut dir = exe.as_path();
        for _ in 0..10 {
            if let Some(parent) = dir.parent() {
                let candidate = parent.join("backend/crates/oqto-browserd");
...
/// from the oqto-browserd directory so Playwright resolves correctly.
