    ...
    match command {
        UserCommand::Create {
            ...
            if !no_linux_user {
                ...
                if user_exists {
                    if !json {
                        println!("Linux user '{}' already exists, skipping creation", linux_username);
                    }
                } else {
                ...
                if !status.success() {
                    eprintln!("Warning: Failed to enable lingering for '{}'", linux_username);
                }
            }
            ...
            if json {
                ...
                println!("  Email: {}", email);
                println!("  Display name: {}", display_name.as_deref().unwrap_or(&username));
                println!("  Role: {}", role);
                if !no_linux_user {
        ...
        UserCommand::List { runner_status } => {
            ...
            if json {
                ...
                if users.is_empty() {
                    ...
                    println!("{:<20} {:<8} {:<15} {}", "USERNAME", "UID", "RUNNER STATUS", "HOME");
                    println!("{}", "-".repeat(70));
                    for user in &users {
                        println!(
                            "{:<20} {:<8} {:<15} {}",
        ...
        UserCommand::Show { user } => {
            ...
            if json {
                ...
                println!("  UID: {}", uid);
                println!("  Home: {}", home);
                println!("  Lingering: {}", if lingering { "enabled" } else { "disabled" });
                println!("\nRunner:");
        ...
        UserCommand::SetupRunner { user, force } => {
            ...
            if std::path::Path::new(&service_path).exists() && !force {
                if json {
                    ...
                    println!("Runner already installed for '{}'. Use --force to reinstall.", user);
                }
                return Ok(());
            }
    ...
    if std::path::Path::new(&socket_path).exists() {
        ...
        let output = std::process::Command::new("sudo")
            .args([
                "machinectl", "shell", 
                &format!("{}@", username),
                "/usr/bin/systemctl", "--user", "is-active", "octo-runner"
            ])
            .output();
...
fn setup_runner_for_user(username: &str, json: bool) -> Result<()> {
    ...
    let status = std::process::Command::new("sudo")
        .args(["-u", username, "systemctl", "--user", "enable", "octo-runner"])
        .status()
    ...
    let status = std::process::Command::new("sudo")
        .args(["-u", username, "systemctl", "--user", "start", "octo-runner"])
        .status()
