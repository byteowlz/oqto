...
impl LocalRuntimeConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if !Self::binary_exists(&self.fileserver_binary) {
            anyhow::bail!(
                "oqto-files binary not found: {}. Build oqto-files or set local.fileserver_binary in config.",
...
mod tests {
    ...
    fn test_local_runtime_config_default() {
        ...
        assert_eq!(config.fileserver_binary, "oqto-files");
        assert_eq!(config.ttyd_binary, "ttyd");
        assert_eq!(config.workspace_dir, "$HOME/oqto/{user_id}");
    ...
    fn test_config_default_serde() {
        ...
        assert_eq!(parsed.fileserver_binary, "oqto-files");
        assert_eq!(parsed.ttyd_binary, "ttyd");
        assert_eq!(parsed.workspace_dir, "$HOME/oqto/{user_id}");
    ...
    fn test_workspace_for_user() {
        let config = LocalRuntimeConfig {
            workspace_dir: "/home/test/oqto/{user_id}".to_string(),
            ..Default::default()
        };

        let path = config.workspace_for_user("alice");
        assert_eq!(path, std::path::PathBuf::from("/home/test/oqto/alice"));

        let path = config.workspace_for_user("bob");
        assert_eq!(path, std::path::PathBuf::from("/home/test/oqto/bob"));
    }
    ...
    fn test_workspace_for_user_with_env_var() {
        ...
        assert_eq!(
            path,
            std::path::PathBuf::from(format!("{}/oqto/testuser", home))
        );
    }
    ...
    fn test_workspace_base() {
        ...
        assert_eq!(
            ...
            std::path::PathBuf::from("/home/test/oqto")
        );
    ...
    fn test_workspace_base_with_env_var() {
        ...
        assert_eq!(path, std::path::PathBuf::from(format!("{}/oqto", home)));
    }
    ...
    fn test_config_deserialization_without_linux_users() {
        ...
        assert!(!parsed.linux_users.enabled);
        assert_eq!(parsed.linux_users.prefix, "oqto_");
