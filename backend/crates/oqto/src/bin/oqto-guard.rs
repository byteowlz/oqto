//! oqto-guard - FUSE filesystem for runtime file access control.
//!
...
//! ```bash
//! # Start the guard
//! oqto-guard --mount /tmp/oqto-guard-1000 \
//!            --oqto-server http://localhost:8080
//!
//! # With config
//! oqto-guard --config ~/.config/oqto/sandbox.toml --profile development
//! ```
//!
//! ## How it works
//!
...
//! 4. For "prompt" policy, sends request to oqto server and waits for approval
...
#[command(
    ...
    after_help = "Examples:\n  \
        oqto-guard --mount /tmp/oqto-guard\n  \
        oqto-guard --config ~/.config/oqto/sandbox.toml"
)]
struct Args {
...
impl GuardedFs {
    ...
    /// Request approval from oqto server (blocking)
...
fn main() -> Result<()> {
    ...
    let mount_point = args.mount.unwrap_or_else(|| {
        ...
        PathBuf::from(format!("/tmp/oqto-guard-{}", uid))
    });
    ...
    // Build mount options
    let mut options = vec![
        MountOption::FSName("oqto-guard".to_string()),
