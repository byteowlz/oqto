...
export class BrowserManager {
  ...
  async launch(options: LaunchOptions): Promise<void> {
    ...
    if (hasExtensions) {
      ...
      context = await launcher.launchPersistentContext(
        path.join(os.tmpdir(), `oqto-browserd-ext-${session}`),
        {
          headless: false,
  ...
  async screenshot(options: {
    ...
    if (!savePath) {
      ...
      const screenshotDir = path.join(os.tmpdir(), "oqto-browserd", "screenshots");
  ...
  async startRecording(outputPath: string, url?: string): Promise<void> {
    ...
    const session = process.env.AGENT_BROWSER_SESSION || "default";
    this.recordingTempDir = path.join(os.tmpdir(), `oqto-browserd-recording-${session}-${Date.now()}`);
