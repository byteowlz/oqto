...
impl ScaffoldConfig {
    ...
    /// 1. OQTO_SCAFFOLD_CONFIG environment variable (path to config file)
    /// 2. ~/.config/oqto/scaffold.toml
    /// 3. /etc/oqto/scaffold.toml
    /// 4. Built-in defaults
    pub fn load() -> Result<Self> {
        let mut config = Self::default();

        // Try /etc/oqto/scaffold.toml first (lowest priority)
        let system_config = Path::new("/etc/oqto/scaffold.toml");
        if system_config.exists() {
            debug!("Loading system config from {:?}", system_config);
            config = Self::merge(config, Self::load_file(system_config)?);
        }

        // Try ~/.config/oqto/scaffold.toml
        ...
        // Try OQTO_SCAFFOLD_CONFIG environment variable (highest priority)
        if let Ok(env_path) = std::env::var("OQTO_SCAFFOLD_CONFIG") {
            let path = PathBuf::from(shellexpand::tilde(&env_path).to_string());
            if path.exists() {
                info!("Loading config from OQTO_SCAFFOLD_CONFIG: {:?}", path);
    ...
    fn user_config_path() -> Option<PathBuf> {
        dirs_next::config_dir().map(|p| p.join("oqto").join("scaffold.toml"))
    }
    ...
    pub fn templates_path(&self) -> Option<PathBuf> {
        ...
        if let Ok(path) = std::env::var("OQTO_TEMPLATES_PATH") {
        ...
        // Default location
        dirs_next::data_dir().map(|p| p.join("oqto").join("templates"))
    }
