...
enum Command {
    Hydrate {
        /// Path to the install config (octo.install.toml)
        #[arg(long, default_value = "octo.install.toml")]
...
fn hydrate_configs(install_path: &Path, mode_override: Option<HydrateMode>) -> Result<()> {
    ...
    let targets = [
        (
            "octo",
            install_config.octo,
            config_home.join("octo").join("config.toml"),
        ),
        (
            "sandbox",
            install_config.sandbox,
            config_home.join("octo").join("sandbox.toml"),
        ),
        (
            "hstry",
