...
impl Default for SandboxConfig {
    fn default() -> Self {
        Self {
            ...
            deny_write: vec![
                "~/.config/octo/sandbox.toml".to_string(),
            ],
...
impl SandboxConfig {
    ...
    pub fn load_global() -> Result<Self> {
        ...
        if !config_path.exists() {
            debug!("No global sandbox config at {:?}, using defaults", config_path);
    ...
    pub fn merge_with_workspace(&self, workspace_config: &Self) -> Self {
        ...
        let workspace_allow: HashSet<String> = workspace_config.allow_write.iter().cloned().collect();
    ...
    #[cfg(target_os = "macos")]
    pub fn build_sandbox_exec_args(&self, workspace: &Path) -> Option<(Vec<String>, tempfile::NamedTempFile)> {
