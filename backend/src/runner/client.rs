...
impl RunnerClient {
    ...
    pub async fn subscribe_stdout(
        &self,
        id: impl Into<String>,
    ) -> Result<StdoutSubscription> {
        ...
        match resp {
            RunnerResponse::StdoutSubscribed(_) => {
                Ok(StdoutSubscription {
                    process_id,
                    lines,
                    _writer: writer,
                })
        ...
        match self.lines.next_line().await {
            Ok(Some(line)) => {
                match serde_json::from_str::<RunnerResponse>(&line) {
                    ...
                    Ok(RunnerResponse::StdoutEnd(e)) => {
                        Some(StdoutSubscriptionEvent::End { exit_code: e.exit_code })
                    }
                    Ok(_) => {
