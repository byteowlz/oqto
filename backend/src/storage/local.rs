...
impl Storage for LocalStorage {
    ...
    async fn list(&self, prefix: &str) -> StorageResult<Vec<StorageMetadata>> {
        ...
        if !full_path.is_dir() {
            return Err(StorageError::InvalidPath(format!("{} is not a directory", prefix)));
        }
    ...
    async fn sync_to_storage(&self, local_path: &Path, storage_path: &str) -> StorageResult<()> {
        ...
        debug!("Synced {} to storage {}", local_path.display(), storage_path);
        Ok(())
    }
    ...
    async fn sync_from_storage(&self, storage_path: &str, local_path: &Path) -> StorageResult<()> {
        ...
        debug!("Synced storage {} to {}", storage_path, local_path.display());
        Ok(())
    }
}
...
mod tests {
    ...
    async fn test_read_string() {
        ...
        storage.write_string("test.txt", "hello world").await.unwrap();
    ...
    async fn test_nested_directories() {
        ...
        storage.write("a/b/c/deep.txt", b"deep content").await.unwrap();
