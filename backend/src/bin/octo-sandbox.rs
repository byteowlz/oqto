//! octo-sandbox - Sandbox wrapper for agent processes.
//!
...
//! octo-sandbox --profile minimal -- ./my-script.sh
//!
...
#[cfg(target_os = "linux")]
fn exec_sandboxed(config: &SandboxConfig, command: &[String], workspace: &PathBuf, dry_run: bool) -> Result<()> {
...
fn exec_sandboxed(_config: &SandboxConfig, command: &[String], workspace: &PathBuf, dry_run: bool) -> Result<()> {
...
fn main() -> Result<()> {
    ...
    info!(
        "octo-sandbox: platform={}, profile={}, workspace={:?}, command={:?}",
        std::env::consts::OS,
        config.profile,
        workspace,
        args.command
