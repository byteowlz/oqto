...
- `eavs`, `hstry` start first (no interdependencies)
...
hstry and eavs services.

Memory is not a service: the oqto backend embeds `mmry-core` and reads/writes
each workspace's `.mmry/mmry.jsonl` directly (ADR-0010). `mmry` on disk is the
lean append-only CLI, not a daemon.
...
systemctl --user enable --now hstry eavs oqto-runner
```
...
```bash
# Enable all services
systemctl --user enable --now hstry eavs oqto-runner oqto

# Check status
systemctl --user status hstry eavs oqto-runner oqto
```
...
- mmry: `~/.config/mmry/config.toml` (lean CLI; workspace memory lives in each workspace's `.mmry/mmry.jsonl`)
