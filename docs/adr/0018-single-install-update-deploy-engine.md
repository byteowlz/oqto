...
- **Deployment composition is explicit, not frontend-universal** (clarification 2026-09-25). A full personal/team WebUI target needs its backend/frontend and declared service dependencies; a **runner-only** target runs headlessly with its runner transport, working Pi runtime and usable model/provider, sandbox and durable `oqto-log` history. Pi chat is mandatory for this target; "runner-only" means without the WebUI/control-plane backend, not a Files-only or chat-disabled runner. A runner-only target must not be forced to install frontend, `oqto.service`, browser assets, local EAVS when an external provider is configured, or hstry. Both targets use this *same* manifest-driven acquisition and transactional activation engine; the declared target determines asset requirements and strict-doctor checks. Absent assets must never silently select or upgrade a target. This composition and its target-specific doctor are **not implemented yet** (`oqto-vemr.9.1.2`); current personal/team manifest checks remain mandatory for their existing target.
...
- **Artifact targets must match an implemented native installer contract.** `oqto-setup install` refuses unsupported macOS releases and mismatched Linux CPU targets before creating a release root. Dependency `deps` planning and artifact-only `acquire` may target another host, but `acquire --install-bin` refuses non-native or unsupported target **before** manifest read, download or local entrypoint mutation (`oqto-vemr.9.4`). Mac archive producers set `COPYFILE_DISABLE=1` so AppleDouble metadata does not escape the single release root; extraction still rejects undeclared members. macOS launchd/installer, signing, and strict doctor are separate future work, not implied by an arm64 build (`oqto-vemr.8.4`).
- **The embedded-installer bootstrap has a pre-execution trust boundary** (clarification 2026-09-25, partial implementation `oqto-vemr.9.2`). Because `oqto-setup` cannot validate itself before it is extracted from a release archive, one small standard-library bootstrap checks the independently obtained per-artifact SHA-256 and canonical full layout, accepts only bounded regular members, and writes exactly one installer into an owner-private directory. The matrix and deploy reuse this boundary; neither may run wildcard tar extraction or select an installer with `find`. After bootstrap, only `oqto-setup` owns activation. This prevents unchecked extraction/execution, **not** source-authenticity or signing; release signatures and native local/remote deploy proof remain mandatory. Separately, `oqto-setup` uses one bounded tar extractor for release and checksummed tool archives, rather than an elevated external tar extraction for tool acquisition.
