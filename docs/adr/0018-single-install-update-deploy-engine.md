...
- **Artifact targets must match an implemented native installer contract.** `oqto-setup install` refuses unsupported macOS releases and mismatched Linux CPU targets before creating a release root. Mac archive producers set `COPYFILE_DISABLE=1` so AppleDouble metadata does not escape the single release root; extraction still rejects undeclared members. macOS launchd/installer, signing, and strict doctor are separate future work, not implied by an arm64 build (`oqto-vemr.8.4`).
