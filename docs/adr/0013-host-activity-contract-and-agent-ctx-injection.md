...
- **Location:** default is a derived convention, `$XDG_STATE_HOME/agent-activity/<harness>/<session_id>.json` (no env var needed in the common case). `AGENT_ACTIVITY_DIR` is an optional override knob only (e.g. to point at an external collector's path). The whole activity convention — the var, the default path, the heartbeat file schema, cadence, and expiry — is owned entirely by **gvnr** (the collector); it is deliberately NOT added to the agent-context-env spec, which stays identity-only. Emitters carry `AGENT_CTX` values in the payload but otherwise just honor gvnr's convention. **Whichever path is used must be added to sandbox `allow_write`** — `~/.local/state` is not writable in the development profile today (blocker).
