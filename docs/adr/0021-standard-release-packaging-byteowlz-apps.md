...
**Implementation-status correction (2026-09-25; not a change to the decision):** the published v0.5.0 `release.yml` bundle has the old flat `bin/` layout, a separate frontend asset, and an embedded installer requiring GLIBC_2.39 (incompatible with Ubuntu 22.04). Canonical local `scripts/dist/package.sh` instead stages `immutable/bin/` with a manifest; its manifest still omits the deployable frontend, Pi runtime, and system units needed for a strict clean install. The two artifact paths are not interchangeable or complete today. This historical rollout claim is not a release-readiness proof; see `oqto-vemr.9.1` and the Ubuntu matrix evidence in `oqto-vemr.8`.
