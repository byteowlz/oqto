...
- 2026-03-12: Improve SearXNG service resilience against cache write failures by updating the generated `searxng-launch` wrapper (user + system services) to use a dedicated writable runtime temp directory (`$SEARXNG_BASE/tmp`) and clear stale sqlite cache files before startup instead of relying on `/tmp` defaults.
- 2026-03-12: Add SearXNG health watchdog (`searxng-healthcheck.service` + `searxng-healthcheck.timer`) for both user/system installs. The probe now validates `/search?format=json`, checks for crashed-engine/readonly-db signals, and auto-restarts `searxng` when degraded.
- 2026-03-12: Stabilize session title rendering to prevent regressions back to "New Session" after a generated title exists by preserving prior non-generic titles when incoming updates are fallback placeholders (`New Session` / `active session` variants). Added frontend regression test `frontend/tests/session-title-stability.test.ts`.
- 2026-03-12: Harden runner Pi stdout handling for oversized event lines by switching from `BufReader::lines()` to `read_until('\n')` in `pi_manager` (avoids line-length cap induced false process-exit handling on large tool payload deltas). Also suppress misleading `No response for 120s` errors while sessions are in `Stopping`; now force `Idle` without surfacing a timeout error.
- 2026-03-12: Add session-target self-healing in WS routing/terminal resolution: if a persisted session target is marked `personal` but its `workspace_path` resolves to a shared workspace, auto-correct it to `shared_workspace` and route commands/terminal via the shared runner. This fixes sessions where messages silently failed due to stale target metadata.
- 2026-03-12: Fix shared-workspace file writes/uploads in WS files channel by resolving the effective Linux user from workspace target (shared workspace owner) instead of always using the requesting personal user runner. Also surface backend files error text in frontend `writeFileMux` instead of generic "Failed to write file".
- 2026-03-12: Strengthen session/runner target invariants to prevent personal/shared drift: `chat_session_targets` upserts now validate scope/path consistency (reject personal rows with shared workspace paths; require `workspace_id` for shared scope), non-create WS agent commands fail closed if target cannot be resolved (no implicit personal-runner fallback), and `session.create` responses now include explicit target metadata (`target_scope`, `target_workspace_id`, `workspace_path`) for frontend-side target awareness.
- 2026-03-11: Add `read-file-guard` Pi extension to truncate oversized `read` tool text payloads (including large PDF-derived text) with a bounded preview and guidance for chunked reads/conversion, preventing provider request-body overflows and session instability. Also include `read-file-guard` + `read-image-guard` in usermgr default extension installs for new users.
- 2026-03-11: Setup now ensures global Python PDF tooling for agent workflows on each installation using `uv tool install` (with `pdfplumber` and `pymupdf`) in prerequisites, so PDF analysis sessions have required dependencies by default without modifying system site-packages.
- 2026-03-11: Strengthen agent policy docs to enforce `uv`-first Python package management (`AGENTS.md` + embedded bootstrap `AGENTS.md`): agents must not use `pip` directly and should prefer `uv add` / `uv tool install` / `uv run` / `uv sync`.
