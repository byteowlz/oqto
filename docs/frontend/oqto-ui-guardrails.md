...
- `compositor` imports only `compositor` and `platform`. Inside it, `kernel/` is the framework-independent Layout Engine (ADR-0041): it imports only kernel-relative modules — no packages, aliases, host globals, adapters, or nondeterminism (`Math.random`, `Date.now`, `new Date`, `performance.now`) — and everything outside `kernel/` reaches it only through the explicit seam files `compositor/index.ts` (transactions, geometry, projection, persistence, presets) and `compositor/navigation.ts` (directional navigation queries). `any` is banned everywhere in `compositor`, layout serialization lives only in the versioned `kernel/persistence` codec, and the compositor may not depend on the legacy App presentation adapter.
