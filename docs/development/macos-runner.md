...
**Proposed network Files grant (not deployed):** [ADR-0051](../adr/0051-runner-owned-network-filesystem-grants.md) makes this TLS endpoint inventory-only unless the dedicated single-user runner explicitly configures `[runner] remote_roots = ["/canonical/absolute/work-root"]`. It bounds and confines mediated Files operations; it does not authorize Pi/process commands. A root containing the real home or `/` includes secrets—there is no implicit "minus SSH keys" carve-out. Keep the live Mac config unchanged until the isolated native and negative probes pass, then use backup + diff + merge, not a config rewrite. Revocation requires a service restart and fresh connection.
